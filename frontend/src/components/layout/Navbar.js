...
import { Bars3Icon, XMarkIcon, UserIcon, Cog6ToothIcon, ArrowRightOnRectangleIcon, UsersIcon } from '@heroicons/react/24/outline';
    ...
    ...(isAuthenticated() && user?.role === 'admin' ? [
      {
        name: 'Admin Panel',
        items: [
          { name: 'Pending Students', href: '/admin/pending-students', icon: UsersIcon },
          { name: 'Add Tutor', href: '/admin/add-tutor', icon: UserIcon },
        ]
      },
                    ...
                    {navigation.map((item) =>
                      item.items ? (
                        <Menu as="div" className="relative" key={item.name}>
                          ...
                          >
                            <Menu.Items className="absolute left-0 z-10 mt-2 w-48 origin-top-right rounded-xl bg-white/95 backdrop-blur-lg py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
